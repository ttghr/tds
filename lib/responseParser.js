...
var responseParser =  function (connected) {
	...
	while (!rowParsed) {
		...
		switch (token) {
			...
			case 0xaa:
				var tokenLen = data.readUInt16LE(++offset, false);
				offset += 2;
				offset = this.parseErrorMessage(data, offset);
				rowParsed = true;
			break;
			...
			//only info
				...
				if(doneRes.moreDone){
					...
					//special case: starting with an empty resultset
					if (token == 0x00 && data[offset+1] == 0x81){
						token=data[++offset];
					}
